...
async function cleanup() {
    ...
    if (!admin) {
        ...
        if (createError || !newUser.user) throw createError || new Error("Failed to create admin");
    ...
    } catch (e) { console.error("Error deleting messages:", e); }

    for (const table of tablesToClear) {
        if (!table.ownerCol) continue;
