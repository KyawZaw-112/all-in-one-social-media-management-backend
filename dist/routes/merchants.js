 ...
 */
router.patch("/toggle-auto-reply", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
...
router.get("/orders", requireAuth, async (req, res) => {
    try {
        ...
        if (query.error) {
            ...
            if (query.error.code === '42P01' || query.error.message?.includes('does not exist'))
 ...
 */
async function handleStatusApproved(conversationId, merchantId, message) {
    try {
