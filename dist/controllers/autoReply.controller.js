...
export class AutoReplyFeatureController {
    ...
    async getStats(req, res) {
        try {
            ...
            const stats = {
                ...
                breakdown: {
                    by_status: {
                        received: messages?.filter((m) => m.status === 'received').length || 0,
                        replied: messages?.filter((m) => m.status === 'replied').length || 0,
                        skipped: messages?.filter((m) => m.status === 'skipped').length || 0,
                    },
                    by_channel: {
    ...
    evaluateCondition(condition, message) {
        if (condition.contains_keywords) {
            const keywords = condition.contains_keywords;
            return keywords.some(keyword => message.body?.toLowerCase().includes(keyword.toLowerCase()));
        }
